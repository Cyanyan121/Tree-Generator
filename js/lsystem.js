...
function drawTree() {
     ...
     **/
    var randomfrequency = parseFloat($("#random-frequency").val());
    var randomstrength = parseFloat($("#random-strength").val());
    // Refresh random frequency
    var reRandomFrequency = document.getElementById("randomFrequency");
    var getBaseFrequency = reRandomFrequency.getAttribute("baseFrequency");
    reRandomFrequency.setAttribute("baseFrequency", randomfrequency);    
    // Refresh random strength
    var reRandomStrength = document.getElementById("randomStrength");
    var getScale = reRandomStrength.getAttribute("scale");
    reRandomStrength.setAttribute("scale", randomstrength);     
...
function drawLSystem(turtle, string, length, angle, width,
    ...
    for (var i = 0; i < string.length; i++) {
        ...
        if ("ABCDEF".indexOf(symbol) >= 0) {
             ...
             **/
            var temp_color = "black"; // default color
            if(colors.hasOwnProperty(symbol))
                temp_color = colors[symbol]; // assigned color
